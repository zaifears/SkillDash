...
const verifyEmailVerified = async (userId: string): Promise<boolean> => {
    try {
        ...
        console.log(`🔍 [API] Email verified check for ${userId.substring(0, 8)}...: ${userRecord.emailVerified}`);
        return userRecord.emailVerified || false;
    } catch (error) {
        console.error('❌ [API] Error checking email verification:', error);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        try {
            ...
            if (!isEmailVerified) {
                console.log(`🚫 [Discover API] Blocked unverified user: ${userId.substring(0, 8)}...`);
                return NextResponse.json({
            ...
            console.log(`✅ [Discover API] Email verified user: ${userId.substring(0, 8)}...`);
        } catch (verificationError) {
            console.error('❌ [Discover API] Email verification check failed:', verificationError);
            return NextResponse.json({
