  ...
  const handleApprove = async (requestId: string, userId: string, coins: number, userName: string) => {
    ...
    try {
      ...
      try {
        ...
        errors.push(`Failed to update request: ${err.message}`);
      }
      ...
      try {
        ...
        errors.push(`Failed to credit coins: ${err.message}`);
      }

      // 3. Try to create transaction record (optional, silent fail is OK)
      try {
        ...
        // Silent fail - transaction logging is optional
        console.warn('Transaction logging failed (this is OK):', transactionError.message);
      }

      // Show result based on what succeeded
      if (successfulOperations >= 2) {
        // Both critical operations succeeded
