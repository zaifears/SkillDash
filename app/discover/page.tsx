...
export default function DiscoverPage() {
  ...
  const [isInputDisabled, setIsInputDisabled] = useState(true); // ✅ FIX: Start disabled - only enable after coin validation
    ...
    const performCoinCheck = async () => {
      if (user && !coinCheckAttempted.current) {
        ...
        try {
          ...
          const actualHasEnough = currentBalance >= LIMITS.COINS_PER_FEATURE;
          
          console.log(`💰 [DiscoverPage] Coin validation: ${currentBalance} >= ${LIMITS.COINS_PER_FEATURE} = ${actualHasEnough}`);
