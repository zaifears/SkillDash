...
// 🔒 DEPRECATED: In-memory rate limiting (doesn't survive restarts)
// DO NOT USE - use checkPersistentRateLimit from lib/utils/persistentRateLimit.ts instead
const rateLimitDeprecated = (() => {
...
/**
 * 🔒 DEPRECATED: Rate limiting middleware (uses in-memory storage)
 * DO NOT USE IN PRODUCTION - use checkPersistentRateLimit instead
 * 
 * This will reset on server restart and doesn't work across load balancers
 */
export const checkRateLimit = (req: NextRequest, identifier?: string): boolean => {
  const clientIP = identifier || getClientIP(req);
  return rateLimitDeprecated(clientIP);
};
